// screen.js - Handles screen capture functionality
...
export class ScreenManager {
    ...
    async loadImageAsDataURL() {
        return fetch(this.loadingImagePath)
            ...
            .catch(error => console.error('Error loading image as DataURL:', error));
    }
    ...
    handleScreenCapture(response) {
        ...
        if (!response || !response.success) {
            console.error('Screen: Invalid screen capture response:', response.message || 'Unknown error');
            return;
        }
        ...
        if (preview) {
            ...
            if (response.fullData && response.fullData.startsWith('data:image')) {
            ...
            console.log('Screen: Preview updated with src:', img.src.substring(0, 30) + '...');
        } else {
    ...
    async processImage(imageData) {
        ...
        try {
            ...
            console.log('Screen: Image loaded successfully, dimensions:', img.width, 'x', img.height);
    ...
    async initialize() {
        ...
        try {
            ...
            if (preview) {
                ...
                img.onerror = (e) => {
                    console.error('Screen: Error loading initial preview image:', e);
                ...
                console.warn('Screen: Preview element not found during initialization');
            }

            this.isInitialized = true;
            console.log('Screen: Screen capture initialized');
        } catch (error) {
            console.error('Screen: Failed to initialize screen capture:', error);
    ...
    async capture() {
        ...
        try {
            ...
            if (this.extensionId) {
                console.log('Screen: Requesting screenshot from extension:', this.extensionId);

                console.log('Screen: Sending chrome.runtime.sendMessage to extension ID:', this.extensionId);
                ...
                chrome.runtime.sendMessage(
                    ...
                    (response) => {
                        console.log('Screen: Received response from extension:', response);

                        if (chrome.runtime.lastError) {
                            console.error('Screen: Extension error details:', chrome.runtime.lastError);
                            window.postMessage({
                        ...
                        if (response && response.success) {
                            console.log('Screen: Processing successful screenshot response');
                            ...
                            // If response has data and fullData properties (new format from ScreenshotHandler)
                                ...
                                data = response.data;
                                fullData = response.fullData || ('data:image/png;base64,' + data);
                            }
                            // Fallback if no recognizable format
                            else {
                                console.warn('Screen: Unrecognized response format, using test image');
                            ...
                            console.error('Screen: Screenshot failed:', response ? response.message : 'Unknown error');
                            window.postMessage({
                                type: 'SCREENSHOT_RESULT',
                                success: false,
                ...
                console.warn('Screen: Extension ID not available, using test image');
                return this.processImage(this.loadingImageDataUrl);
            }
        } catch (error) {
